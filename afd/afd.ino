    ...
    void connectWiFi() {
      ...
      const unsigned long WIFI_TIMEOUT_MS = 15000UL; // 15 second timeout
      ...
      while (WiFi.status() != WL_CONNECTED) {
        if (millis() - startMs > WIFI_TIMEOUT_MS) {
          ...
          switch (WiFi.status()) {
            case WL_NO_SSID_AVAIL: Serial.println("SSID not found (check name or use 2.4GHz band)"); break;
            case WL_CONNECT_FAILED: Serial.println("Wrong password"); break;
            case WL_DISCONNECTED:   Serial.println("Disconnected / unreachable"); break;
